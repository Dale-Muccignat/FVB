...
@fastmath function LinearAlgebra.ldiv!(F::SparseArrays.SPQR.QRSparse{Float64, Int64}, B::Vector{Float64}) @inbounds begin
...
function pt(gas;T,E,εmax=NaN,μ=NaN,lmax=1,N=200,superelastic=false,supertype=:tseparate,verbose=false) @inbounds begin
    ...
    for ioloss in gas.iolosses if ioloss < εmax
        siz += (1+N÷2)N
    end end
    if μ != 0
        siz += (lmax+1)N
    ...
    is = Vector{Int}(undef,siz)
    js = Vector{Int}(undef,siz)
    Ls = Vector{Float64}(undef,siz)

    c = 0
    c += 1; is[c] = 1; js[c] = 1; Ls[c] = -(νₗₒₛₛ[1] + Ab[1]/Δv)
    c += 1; is[c] = 1; js[c] = 2; Ls[c] = -Bb[1]/Δv
    for i = 2:N-1
        c += 1; is[c] = i; js[c] = i-1; Ls[c] = Ab[i-1]/Δv
        c += 1; is[c] = i; js[c] = i; Ls[c] = -(νₗₒₛₛ[i] + (Ab[i] - Bb[i-1])/Δv)
        c += 1; is[c] = i; js[c] = i+1; Ls[c] = -Bb[i]/Δv
    end
    ...
    # TODO: Superelastic does not handle mixtures...
    ...
    if supertype === :tseparatebolsig
        for j in 1:no_gases
            for k in eachindex(gas.excitation) if gas.excitation[k].type ∈ [:ROTATION] && gas.ex_nos[k] == j
    ...
    for ng in 1:no_gases
        for t in ex_types
            Z_t = 1. 
            for k in eachindex(gas.excitation) if gas.excitation[k].type === t && gas.ex_nos[k] == ng
                Z_t += Z(gas.exlosses[k],T_og)
                for ki in eachindex(gas.excitation) if ki != k && gas.excitation[ki].type === t && gas.ex_nos[ki] == ng
    ...
    for k in eachindex(gas.ionisation)
        ...
        for i = 1:N
            ...
            w = 1/2+(v[j]-vshift)/Δv
            h = w > 0 ? j : j+1
            c += 1; is[c] = i; js[c] = h; Ls[c] = w*prefactor*νᵢₒ(v[h])/(ε[h]-εₖ)
            for h = j+1:N
                c += 1; is[c] = i; js[c] = h; Ls[c] = prefactor*νᵢₒ(v[h])/(ε[h]-εₖ)
            end
        end
    end
    ...
    # L01
    l = 0
    c += 1; is[c] = l*N+1; js[c] = (l+1)*N+1; Ls[c] = +E*(l+1)/2(2l+3)*(1/Δv)
    for i = 2:N-1
        c += 1; is[c] = l*N+i; js[c] = (l+1)*N+i-1; Ls[c] = +E*(l+1)/2(2l+3)*(l/2v[i]-1/Δv)
        c += 1; is[c] = l*N+i; js[c] = (l+1)*N+i; Ls[c] = +E*(l+1)/2(2l+3)*(l/2v[i]+1/Δv)
    end
    c += 1; is[c] = l*N+N; js[c] = (l+1)*N+N-1; Ls[c] = +E*(l+1)/2(2l+3)*(-1/Δv)

    # L10
    l = 1
    c += 1; is[c] = l*N+1; js[c] = (l-1)*N+1; Ls[c] = -E*l/2(2l-1)*((l+1)/2vb[1])
    c += 1; is[c] = l*N+1; js[c] = (l-1)*N+1+1; Ls[c] = -E*l/2(2l-1)*((l+1)/2vb[1]-1/Δv)
    for i = 2:N-1
        c += 1; is[c] = l*N+i; js[c] = (l-1)*N+i; Ls[c] = -E*l/2(2l-1)*((l+1)/2vb[i]+1/Δv)
        c += 1; is[c] = l*N+i; js[c] = (l-1)*N+i+1; Ls[c] = -E*l/2(2l-1)*((l+1)/2vb[i]-1/Δv)
    end
    c += 1; is[c] = l*N+N; js[c] = (l-1)*N+N; Ls[c] = -E*l/2(2l-1)*((l+1)/2vb[N]+1/Δv)

    # L11
    for i = 1:N
        c += 1; is[c] = l*N+i; js[c] = l*N+i; Ls[c] = -νₜₒₜb[i]
    end

    # Multiterms
    for l = 1:2:lmax-2
        c += 1; is[c] = l*N+1; js[c] = (l+1)*N+1; Ls[c] = +E*(l+1)/2(2l+3)*(l/2vb[1])
        c += 1; is[c] = l*N+1; js[c] = (l+1)*N+1+1; Ls[c] = +E*(l+1)/2(2l+3)*(l/2vb[1]+1/Δv)
        for i = 2:N-1
            c += 1; is[c] = l*N+i; js[c] = (l+1)*N+i; Ls[c] = +E*(l+1)/2(2l+3)*(l/2vb[i]-1/Δv)
            c += 1; is[c] = l*N+i; js[c] = (l+1)*N+i+1; Ls[c] = +E*(l+1)/2(2l+3)*(l/2vb[i]+1/Δv)
        end
        c += 1; is[c] = l*N+N; js[c] = (l+1)*N+N; Ls[c] = +E*(l+1)/2(2l+3)*(l/2vb[N]-1/Δv)

        l += 1
        c += 1; is[c] = l*N+1; js[c] = (l-1)*N+1; Ls[c] = -E*l/2(2l-1)*((l+1)/2v[1]-1/Δv)
        for i = 2:N-1
            c += 1; is[c] = l*N+i; js[c] = (l-1)*N+i-1; Ls[c] = -E*l/2(2l-1)*((l+1)/2v[i]+1/Δv)
            c += 1; is[c] = l*N+i; js[c] = (l-1)*N+i; Ls[c] = -E*l/2(2l-1)*((l+1)/2v[i]-1/Δv)
        end
        c += 1; is[c] = l*N+N; js[c] = (l-1)*N+N-1; Ls[c] = -E*l/2(2l-1)*((l+1)/2v[N]+1/Δv)
        c += 1; is[c] = l*N+N; js[c] = (l-1)*N+N; Ls[c] = -E*l/2(2l-1)*((l+1)/2v[N])
        for i = 1:N
            c += 1; is[c] = l*N+i; js[c] = l*N+i; Ls[c] = -νₜₒₜv[i]
        end
        c += 1; is[c] = l*N+1; js[c] = (l+1)*N+1; Ls[c] = +E*(l+1)/2(2l+3)*(l/2v[1]+1/Δv)
        for i = 2:N-1
            c += 1; is[c] = l*N+i; js[c] = (l+1)*N+i-1; Ls[c] = +E*(l+1)/2(2l+3)*(l/2v[i]-1/Δv)
            c += 1; is[c] = l*N+i; js[c] = (l+1)*N+i; Ls[c] = +E*(l+1)/2(2l+3)*(l/2v[i]+1/Δv)
        end
        c += 1; is[c] = l*N+N; js[c] = (l+1)*N+N-1; Ls[c] = +E*(l+1)/2(2l+3)*(l/2v[N]-1/Δv)
        c += 1; is[c] = l*N+N; js[c] = (l+1)*N+N; Ls[c] = +E*(l+1)/2(2l+3)*(l/2v[N])
        ...
        c += 1; is[c] = l*N+N; js[c] = (l-1)*N+N; Ls[c] = -E*l/2(2l-1)*((l+1)/2vb[N]+1/Δv)
        for i = 1:N
            c += 1; is[c] = l*N+i; js[c] = l*N+i; Ls[c] = -νₜₒₜvb[i]
        end
    end

    if μ != 0
        for i = 1:(lmax+1)N
            c += 1; is[c] = i; js[c] = i; Ls[c] = -μ
        end
    end
...
#    return L, F, G
    ...
    G[N], B = G[N], (G[N] = 0; Fᴴᴼᴴ\G)

    # Building G^(T) system
    c = 0
    for i = 1:N
        c += 1; is[c] = i; js[c] = i; Ls[c] = 1
    end

    # L10
    l = 1
    c += 1; is[c] = l*N+1; js[c] = (l-1)*N+1; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2vb[1])
    c += 1; is[c] = l*N+1; js[c] = (l-1)*N+1+1; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2vb[1]-1/Δv)
    for i = 2:N-1
        c += 1; is[c] = l*N+i; js[c] = (l-1)*N+i; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2vb[i]+1/Δv)
        c += 1; is[c] = l*N+i; js[c] = (l-1)*N+i+1; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2vb[i]-1/Δv)
    end
    c += 1; is[c] = l*N+N; js[c] = (l-1)*N+N; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2vb[N]+1/Δv)

    # L11
    for i = 1:N
        c += 1; is[c] = l*N+i; js[c] = l*N+i; Ls[c] = -ω₀ - νₜₒₜb[i]
    end

    # Multiterms
    for l = 1:2:lmax-2
        c += 1; is[c] = l*N+1; js[c] = (l+1)*N+1; Ls[c] = E*(l+2)/2(2l+3)*(l/2vb[1])
        c += 1; is[c] = l*N+1; js[c] = (l+1)*N+1+1; Ls[c] = E*(l+2)/2(2l+3)*(l/2vb[1]+1/Δv)
        for i = 2:N-1
            c += 1; is[c] = l*N+i; js[c] = (l+1)*N+i; Ls[c] = E*(l+2)/2(2l+3)*(l/2vb[i]-1/Δv)
            c += 1; is[c] = l*N+i; js[c] = (l+1)*N+i+1; Ls[c] = E*(l+2)/2(2l+3)*(l/2vb[i]+1/Δv)
        end
        c += 1; is[c] = l*N+N; js[c] = (l+1)*N+N; Ls[c] = E*(l+2)/2(2l+3)*(l/2vb[N]-1/Δv)

        l += 1
        c += 1; is[c] = l*N+1; js[c] = (l-1)*N+1; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2v[1]-1/Δv)
        for i = 2:N-1
            c += 1; is[c] = l*N+i; js[c] = (l-1)*N+i-1; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2v[i]+1/Δv)
            c += 1; is[c] = l*N+i; js[c] = (l-1)*N+i; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2v[i]-1/Δv)
        end
        c += 1; is[c] = l*N+N; js[c] = (l-1)*N+N-1; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2v[N]+1/Δv)
        c += 1; is[c] = l*N+N; js[c] = (l-1)*N+N; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2v[N])
        for i = 1:N
            c += 1; is[c] = l*N+i; js[c] = l*N+i; Ls[c] = -ω₀ - νₜₒₜv[i]
        end
        c += 1; is[c] = l*N+1; js[c] = (l+1)*N+1; Ls[c] = E*(l+2)/2(2l+3)*(l/2v[1]+1/Δv)
        for i = 2:N-1
            c += 1; is[c] = l*N+i; js[c] = (l+1)*N+i-1; Ls[c] = E*(l+2)/2(2l+3)*(l/2v[i]-1/Δv)
            c += 1; is[c] = l*N+i; js[c] = (l+1)*N+i; Ls[c] = E*(l+2)/2(2l+3)*(l/2v[i]+1/Δv)
        end
        c += 1; is[c] = l*N+N; js[c] = (l+1)*N+N-1; Ls[c] = E*(l+2)/2(2l+3)*(l/2v[N]-1/Δv)
        c += 1; is[c] = l*N+N; js[c] = (l+1)*N+N; Ls[c] = E*(l+2)/2(2l+3)*(l/2v[N])
        ...
        c += 1; is[c] = l*N+N; js[c] = (l-1)*N+N; Ls[c] = E*(1-l)/2(2l-1)*((l+1)/2vb[N]+1/Δv)
        for i = 1:N
            c += 1; is[c] = l*N+i; js[c] = l*N+i; Ls[c] = -ω₀ - νₜₒₜvb[i]
        end
    end
